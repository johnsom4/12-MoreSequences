...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(100.0, 50.0),  # All numbers are approximate.
                rg.Point(75.0, 93.3),
                rg.Point(25.0, 93.3),
                rg.Point(0.0, 50.0),
                rg.Point(25.0, 6.7),
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 4
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  12 magenta dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(300, 300), 150)
    draw_points_on_circle(window, circle, 12, 'magenta')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  72 slices, thin (thickness=2) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(300, 200), 300)
    circle.outline_thickness = 3
    pizza(window, circle, 72, 'red', 2)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        lin = rg.Line(rg.Point(circle.center.x, circle.center.y), points[k])
...
def run_test_polygon():
    ...
    polygon(window, circle, 8, 'purple', 2)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  10 dark blue lines on red circle,'
             + ' hops = 8.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'red'
    fancy_polygon(window, circle, 10, 8, 'dark blue', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        w = rg.Line(points[k-hops_to_next_point], points[k])
        w.color = color
